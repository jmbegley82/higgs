...
int initField(int count) {
	...
	*/
	...
	setSpriteCount(0);
	retval = count;
	lprintf(LOG_DEBUG, DBGFORM"created _field with %d items\n", DBGSPEC, count);
	return retval;
}
...
bool addRandoToField() {
	...
	pthread_mutex_lock(&_field_mutex);
		...
		lprintf(LOG_DEBUG, DBGFORM"_field[%d].identity is %s\n", DBGSPEC,
...
bool addSpriteToField(char* identity, char* type, double x, double y) {
	lprintf(LOG_DEBUG, DBGFORM"%s, %s, %f, %f\n", DBGSPEC, identity, type, x, y);
	...
	pthread_mutex_lock(&_field_mutex);
		...
		lprintf(LOG_DEBUG, DBGFORM"_field[%d].identity is %s; .anims.identity is %s\n", DBGSPEC,
...
bool delSpriteByIndexUnsafe(int idx) {
	...
	lprintf(LOG_DEBUG, DBGFORM"killing %s, x=%f, y=%f\n", DBGSPEC,
...
bool makeContiguousUnsafe() {
	...
	lprintf(LOG_DEBUG, DBGFORM"oldCount=%d, newCount=%d\n", DBGSPEC, oldCount, newCount);
